...
router.post('/', protect, admin, async (req, res) => {
  ...
  if (!name || !price || !category || !brand) {
    return res.status(400).json({ message: 'Please provide all required fields (name, price, category, brand)' });
  }
...
router.put('/:id', protect, admin, async (req, res) => {
  ...
  try {
    ...
    if (product) {
      ...
      product.countInStock = countInStock !== undefined ? countInStock : product.countInStock; // Allow countInStock to be 0
